...
contract PositionAaveV3Mainnet is UniversalPosition, DeployMainnetContracts {
  // aave v3 connector integration
  ...
  function test_OpenPosition_ClosePosition() public {
    (DataTypes.Position memory position, uint256 index) = openPosition(
      user, getToken("weth"), getToken("dai"), 0.1 ether, 1000 ether, address(sDAIConnector), address(aaveV3Connector)
    );

    closePosition(user, index, address(sDAIConnector), address(aaveV3Connector), position);
  }
  ...
  function test_OpenAndClose_TwoPosition() public {
    ...
    closePosition(user, index1, address(sDAIConnector), address(aaveV3Connector), position1);
    closePosition(user, index2, address(sDAIConnector), address(aaveV3Connector), position2);
  }
  ...
  function _getBorrowCallData(address token, uint256 amount) internal view returns (bytes memory) {
    return abi.encodeWithSelector(aaveV3Connector.borrow.selector, token, amount, RATE_TYPE);
  }
